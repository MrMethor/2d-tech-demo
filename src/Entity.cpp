...
void Entity::updateVelocity(char movingX, char movingY) {
    ...
    if (degree > -1) {
        ...
        maxVelX = sin(axis);
        maxVelY = cos(axis);
        maxVelAbsX = std::abs(maxVelX);
        maxVelAbsY = std::abs(maxVelY);
    ...
    // If player is not moving or is moving faster than movement speed allows him, it slows down velocity
